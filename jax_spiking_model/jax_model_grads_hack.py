...
import neuron_model
from jax_spiking_model import synapse_activation
from jax_spiking_model import synapse_activation_gradient_fn
from jax_spiking_model import simple_activation
...
def timestep(params, connections, synapse_weights, voltages, refractory_timers, rise_values, iteration, perturbations):
    perturb = perturbations[iteration]
    ...
    synapse_strenghts: jnp.ndarray = (pre_synapse_strengths_act * (synapse_weights + perturb))
...
def run_sim(params, connections, num_neurons, synapse_weights, neurons_to_activate, perturbations):
    ...
    def loop(i, x):
        ...
        neurons, refractory_timers, new_rise_values = timestep(params, connections, synapse_weights, with_input, refractory_timers, rise_values[i], i, perturbations)
...
@jax.jit(static_argnames=["params", "num_neurons"])
def sim_loss(params, connections, num_neurons, synapse_weights, neurons_to_activate, target_voltages, perturbations):
    voltages, _, _ = run_sim(params, connections, num_neurons, synapse_weights, neurons_to_activate, perturbations)
    return jnp.sum((target_voltages - voltages) ** 2)

intermediate_grads = jax.grad(sim_loss, argnums=6)
